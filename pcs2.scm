(defprotocol two-party-ocs basic
	...
	; A attempts a resolution with T, but recevies an aborted message
	(defrole init-aborts-init-continues-init-resolves-a
		(vars (a b t name) (m text))
		(trace
			; initial pcs exchange
			(send (enc m a b t (privk "pcs" a)))
			...
			(send (enc m b a t (privk "pcs" b)) (enc m a b t (privk "sign" a)))
			; but A receive a notice that the session was already aborted
			(recv (enc (enc m a b "abort" (privk a)) (privk t)))
		)
	)
	(defrole init-aborts-init-continues-init-resolves-b
		(vars (a b t name) (m text))
		(trace
			; initial pcs exchange
			(recv (enc m a b t (privk "pcs" a)))
			...
			; main protocol continues
			(recv (enc m a b t (privk "sign" a)))
			; B got his signed contract, so he quits out
		)
	)
	(defrole init-aborts-init-continues-init-resolves-t
		(vars (a b t name) (m text))
		(trace
			; A sent an abort request
			(recv (enc (enc m a b "abort" (privk a)) (pubk t)))
			(send (enc (enc m a b "abort" (privk a)) (privk t)))
			; A tries to resolve
			(recv (enc m b a t (privk "pcs" b)) (enc m a b t (privk "sign" a)))
			; respond with a copy of the response sent earlier
			(send (enc (enc m a b "abort" (privk a)) (privk t)))
		)
	)
	...
	(defrole init-aborts-init-continues-resp-aborts-b
		...
		(trace
			...
			(recv (enc m a b t (privk "sign" a)))
			; B attempts an abort
			(send (enc (enc m b a "abort" (privk b)) (pubk t)))
			; but B receive a notice that the session was already aborted by A
			(recv (enc (enc m a b "abort" (privk a)) (privk t)))
		)
	)
	(defrole init-aborts-init-continues-resp-aborts-t
		...
		(trace
			...
			(send (enc (enc m a b "abort" (privk a)) (privk t)))
			; B tries to abort
			(recv (enc (enc m b a "abort" (privk b)) (pubk t)))
			; respond with a copy of the response sent to A
			(send (enc (enc m a b "abort" (privk a)) (privk t)))
		)
	)
